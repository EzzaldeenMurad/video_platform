                        ...
                        <tr>
                                ...
                                <form method="POST" action="{{route('channels.delete', $user)}}" style="display:inline-block">
                                    ...
                                    @if (auth()->user() != $user && !$user->isSuperAdmin())
                                        <button type="submit" class="btn btn-danger btn-sm"
                                            onclick="return confirm('هل أنت متأكد أنك تريد حذف هذه القناة')"><i
                            ...
                            <td>
                                <form method="POST" action="{{route('channels.block',$user)}}" style="display:inline-block">
                                    ...
                                    @if (auth()->user() != $user && !$user->isSuperAdmin())
                                        @if ($user->block)
                                            <div class="btn btn-warning btn-sm disabled"><i class="fas fa-lock"></i> محظور
                                            </div>
                                        @else
                                            <button type="submit" class="btn btn-warning btn-sm"
                                                onclick="return confirm('هل أنت متأكد أنك تريد حظر هذه القناة')"><i
