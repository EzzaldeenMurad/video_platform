    ...
    <div class="mx-4">
        @if ($videos->count() > 0)
            <div class="row justify-content-center">
                <form class="form-inline col-md-6 justify-content-center" method="POST"
                    action="{{ route('history.distroyAll') }}"
                    onsubmit="return confirm('هل أنت متأكد أنك تريد حذف السجل بشكلٍ كامل؟')">
                    @csrf
                    @method('DELETE')
                    ...
                    <div class="col-sm-6 col-md-4 col-lg-3">
                        <div class="card">
                            <div class="card-icons">
                                @php
                                    $hoursAddZero = sprintf('%02d', $video->hours);

                                    $minuteAddZero = sprintf('%02d', $video->minutes);

                                    $secondsAddZero = sprintf('%02d', $video->seconds);
                                @endphp
                                <a href="/videos/{{ $video->id }}">
                                    <img src="{{ Storage::url($video->image_path) }}" class="card-img-top" alt="...">
                                    <time>{{ $video->hours > 0 ? $hoursAddZero . ':' : '' }}{{ $minuteAddZero }}:{{ $secondsAddZero }}</time>
